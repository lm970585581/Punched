...
function big(){
	...
	ctx.strokeText(txt, centerX, centerY);
	// ctx.restore();
}
...
function drawball(deg){
	...
	balls.forEach(function(ball) {
		...
		ball.angle = ball.angle+deg;
		if(ball.angle>=360){
			ball.angle = 0;
		}
		//绘制大球小球之间的线段
		ctx.moveTo(centerX, centerY);
		var rad = Math.PI * ball.angle / 180;
		...
		if(ball.numStr!="") {
			...
			ctx.strokeStyle = "#fff";
			ctx.fillStyle = "#fff";
			ctx.fillText(ball.numStr, x, y);
			ctx.strokeText(ball.numStr, x, y);
		}
...
function drawWait(){
	...
	waitballs.forEach(function(waitball) {
		...
		ctx.strokeStyle = "#fff";
		ctx.fillStyle = "#fff";
		ctx.fillText(waitball.numStr, waitx, waity);
		ctx.strokeText(waitball.numStr, waitx, waity);
		waity += 3 * ballRadius;
	});
}
...
document.onclick = function(){
	...
	balls.forEach(function(o, index) {
		if(!faild) return;
		if(Math.abs(o.angle - ball.angle) <ballRadius*180/ (lineLen*Math.PI)) {
			state = 0;
			faild = false;
